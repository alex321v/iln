...
public class TelegramIlnBot extends TelegramLongPollingBot {
		...
		if (update.hasMessage() && update.getMessage().hasText()) {
			...
			for (int i = 0; i < response.length; i++) {
				...
				SendMessage message = new SendMessage() // Create a message
														// object object
						.setChatId(chat_id).setText(response[i]);
				try {
	...
	public String getBotToken() {
		//return "405806016:AAF-QgVcjcijshxn2USVGrioV1y5lKjijng"; //gattino
		return "448239661:AAFulFHqoaqw0HR6PC_lQxMyY3sXOYWSEhk"; //topino
	}
	...
	/**
	 * La documentazione di questa funzione e' la stessa di quella di @see
	 * IrcClient
	 */
	...
	private String[] learn2(String toNick, String params) {
		...
		if (newKeyWord.indexOf(nickName) > -1) { // okkio! Stanno tentando
													// l'hack del nome!
