...
public class IrcClient extends GenericClient {
	...
	/**
	 * Costruttore. Il nome del file di log sarà Irc-&lt;nickname&gt;.
	 *
	 * @param aHost
	 *            Il nome dell'host al quale connettersi
	 * @param aPort
	 *            Porta dell'host remoto alla quale connettersi
	 * @param aNick
	 *            Nickname
	 * @param aChan
	 *            Nome del canale al quale effettuare una /JOIN
	 * @param deb
	 *            True se si vuole il log esteso. False altrimenti.
	 * @param aResponse
	 *            Il tipo di personalità desiderata.
	 */
	public IrcClient(String aHost, int aPort, String aNick, String aChan, boolean deb, String aMaster, String aPass) {
	 ...
	 * dove nome_host e' l'host al quale connettersi, numero_porta e' il numero
	 * della porta alla quale connettersi, nick e' il nickname da assumere,
	 * debug_level e' il livello di debug e botMaster il nick di chi ha il controllo
	 * del programma.
	 */
	 ...
	 * 
	 * @param sock
	 *            Un socket aperto e connesso all'Host dell'IRCServer. E' il Socket
	 *            restituito dal metodo connect ereditato da GenericClient.
	 */
	...
	public void kicking() {
		...
		switch (selettore) {
		case 0:
			risposta = "ma dioporco che kickate a fare!?";
			break;
		case 1:
			risposta = "ma ti diverti tanto a kickarmi?";
			break;
		case 2:
			risposta = "mi hai preso per Peregrino su #hackit99 ?";
			break;
		case 3:
			risposta = "ma che banda di azzeccati...";
			break;
		case 4:
			risposta = "ora ti senti realizzat*? kickare ti fa sentire potente?";
			break;
		case 5:
			risposta = "guarda che la violenza di un kick non e' la soluzione.";
			break;
		case 6:
			risposta = "kickate solo perche' non riuscite a trombare a pecora!";
			break;
		case 7:
			risposta = "secondo me kickate o per sport o per insoddisfazione sessuale!";
			break;
		}
	...
	/**
	 * Se autenticazione e join sono andati a buon fine, il client e' entrato nel
	 * canale come un qualsiasi umano. Questo metodo gestisce l'attivita' di
	 * ricevimento dei messaggi dal canale o da query ed invia le risposte.
	 */
	...
	/**
	 * Trattamento di un messaggio-stimolo. Puo' essere o un messaggio proveniente
	 * dal canale o un messaggio proveniente da una query privata.<br>
	 ...
	 *
	 * @param prefix
	 *            Il prefisso dello stimolo. Corrisponde al comando di protocollo
	 *            IRC.
	 * @param params
	 *            Parametri del messaggio: mittente, destinatario messaggio.
	 */
	public void trattaPrivMsg(String prefix, String params) {
		...
		// Acchiappo il primo token, che da protollo IRC e' il destinatario del
		// messaggio.
		...
		if (k.equals(nickName)) { // Se il destinatario e' proprio il nostro nick....
			...
			if (msg.indexOf("TIME") < 0)
				if (learningTable.containsKey(toNick)) // Se il mittente mi sta insegnando qualcosa...
					...
					else if (k2.equals(":stop") && (toNick.equals(master))) { // Se e' il comando di arresto
																				// programma...
						...
						if (toNick.equals(master) && (params.indexOf("c_p") > 0)) {
							...
							if (response != null) {
								...
								if (!response[0].equals("learn")) { // Se la generazione e' andata a buon fine
									...
									out.println("PRIVMSG " + toNick
											+ " :Non so cosa significhi. La frase chiave quale e'?");
									log("[query-" + toNick + "]<" + nickName
											+ "> Non so cosa significhi. La frase chiave quale e'?");
								}
							}
						}
			...
			if (!tok.hasMoreTokens() && // se qualcuno mi sta chiamando....
				...
				case 4:
					...
					break;
				case 5:
					risposta_x = "che cazzo vuoi?";
					break;
				case 6:
					risposta_x = "parla, ti ascolto";
					break;
				case 7:
					risposta_x = "beh?";
					break;
			...
			/* Questa parte e' certamente da migliorare! */
				...
				while (tok.hasMoreTokens() && !flaggino) { // ... scandisco il messaggio per capire
					...
					if (parola.indexOf(nickName) > -1) { // se ce l'hanno con me....
						...
						if ((!response[0].equals("no_response")) && (!response[0].equals(lastRes))) { // fix del loop
																										// bug!
							...
							if (autonom) {
								...
								case 0:
									...
									break;
								case 1:
									str = "ma che fesserie dici...";
									break;
								case 2:
									str = "stai dicendo una cosa che non comprendo";
									break;
								case 3:
									str = "spiegati meglio per cortesia";
									break;
								case 4:
									str = "non capisco, e' senza senso...";
									break;
								case 5:
									str = "secondo me tu stai male, molto male";
									break;
								case 6:
									str = "uhm... noto che sei bravo a dire cose senza senso e senza semantica";
									break;
								}
				...
				if (!flaggino && autonom) {
					...
					flaggino = true;
					response = analisi.generation(params, 1);// chiamo il generatore
					if (!response[0].equals("no_response") && !response[0].equals(lastRes)) { // se ho risposte le do,
																								// se no silenzio
						if (response[0].indexOf("CMD") > -1) {
							response = analisi.esegueComando(response[0], params, 1);
							command = true;
		...
		} else { // attenzione: punto critico. loggare per debuggare. Forse un giorno mostreremo
					// che è inutile!
	 ...
	 * Segue alla lettera il protocollo IRC finche' il server non da il WELCOME.
	 */
	...
	private void learn2(String toNick, String params) {
		...
		if (newKeyWord.indexOf(nickName) > -1) { // okkio! Stanno tentando l'hack del nome!
			...
			log("[query-" + toNick + "]<" + nickName
					+ "> No no non puoi darmi come chiave qualcosa con il mio nome...");
			log("[query-" + toNick + "]<" + nickName
					+ "> poi va a finire che quando la gente mi invoca io rispondo cosi");
			log("[query-" + toNick + "]<" + nickName + "> e mica ci casco... sce'!");
		} else {
			learningTable.put(toNick, newKeyWord);
			out.println("PRIVMSG " + toNick + " :" + newKeyWord
					+ ". Mi dici una bella frase che riguarda ciò? Io la imparo.");
			log("[query-" + toNick + "]<" + nickName + "> " + newKeyWord
					+ ". Mi dici una bella frase che riguarda ciò? Io la imparo.");
		}
	}

	/**
	 * Termine del learning. Invoca l'analizzatore per fissare ciò che si e'
	 * imparato.
	 *
	 * @param toNick
	 *            insegnante
	...
	private void learnEnd(String toNick, String params) {
		...
		if (newKeyWord.equals("") || newPhraseLearned.equals("") || newKeyWord.equals("bestemmia")
				|| newKeyWord.equals("aut") || newKeyWord.equals("noaut")) { // controllo hack comandi
			out.println(
					"PRIVMSG " + toNick + " :Ehi mica vorrai fregarmi con qualche stupido hack... sei un lamer :))");
			log("[query-" + toNick + "]<" + nickName
					+ "> Ehi mica vorrai fregarmi con qualche stupido hack... sei un lamer :))");
		} else {
